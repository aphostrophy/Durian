...
pub type DurianResult<T> = Result<T, Error>;

impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Error::ClientConnectionTerminated => {
                write!(f, "Repository connection dropped for Durian client")
            }
            Error::ClientTimeout => {
                write!(f, "Durian client timeout when connecting to repository")
            }
            Error::ClientRefused => {
                write!(f, "Repository refused connection for Durian client")
            }
